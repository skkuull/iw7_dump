...
f0_local0 = function ( f1_arg0, f1_arg1, f1_arg2 )
	...
	GameType:SubscribeToModel( DataSources.frontEnd.lobby.gameTypeName:GetModel( f1_arg1 ), function ()
		local f2_local0 = DataSources.frontEnd.lobby.gameTypeName:GetValue( f1_arg1 )
		if f2_local0 ~= nil then
			GameType:setText( ToUpperCase( f2_local0 ), 0 )
		end
		local f2_local1 = Engine.GetDvarBool( "scr_boss_battles_enabled" )
		if Engine.IsAliensMode() and Engine.GetDvarBool( "scr_boss_battles_enabled" ) then
			GameType:setText( ToUpperCase( Engine.Localize( "LUA_MENU_ZM_BOSS_BATTLE" ) ), 0 )
		end
	end )
	f1_arg0:addElement( GameType )
	f1_arg0.GameType = GameType
	
end

function CPMatchDetails( menu, controller )
	local self = LUI.UIVerticalNavigator.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 500 * _1080p, 0, 140 * _1080p )
	self.id = "CPMatchDetails"
	self._animationSets = {}
	self._sequences = {}
	...
	MapName:SubscribeToModel( DataSources.frontEnd.lobby.mapName:GetModel( f3_local1 ), function ()
		local f4_local0 = DataSources.frontEnd.lobby.mapName:GetValue( f3_local1 )
		if f4_local0 ~= nil then
			MapName:setText( ToUpperCase( f4_local0 ), 0 )
		end
	end )
...
MenuBuilder.registerType( "CPMatchDetails", CPMatchDetails )
LockTable( _M )
