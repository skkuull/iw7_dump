...
function InGameCurrencyCount( menu, controller )
	local self = LUI.UIElement.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 100 * _1080p, 0, 36 * _1080p )
	self.id = "InGameCurrencyCount"
	self._animationSets = {}
	self._sequences = {}
	...
	CurrencyLogo:setImage( RegisterMaterial( "currency_keys_icon" ), 0 )
	...
	CurrencyCount:SubscribeToModelThroughElement( self, nil, function ()
		...
		f3_local0 = f3_local0:GetValue( f2_local1 )
		if f3_local0 ~= nil then
			CurrencyCount:setText( f3_local0, 0 )
		end
	end )
...
MenuBuilder.registerType( "InGameCurrencyCount", InGameCurrencyCount )
LockTable( _M )
