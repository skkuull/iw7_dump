...
function PostLoadFunc( f1_arg0, f1_arg1, f1_arg2 )
	local f1_local0 = function ()
		...
		if f2_local0.isUnlocked:GetValue( f1_arg1 ) then
	...
	f1_arg0:SubscribeToModelThroughElement( f1_arg0, "isUnlocked", f1_local0 )
	...
	if Engine.IsPC() then
		f1_arg0:addEventHandler( "button_action", function ( f4_arg0, f4_arg1 )
			f1_arg0:dispatchEventToCurrentMenu( {
				name = "classic_weapon_select_alternate"
			} )
		end )
	end
end

function ClassicWeaponUnlockButton( menu, controller )
	local self = LUI.UIButton.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 250 * _1080p, 0, 140 * _1080p )
	self.id = "ClassicWeaponUnlockButton"
...
MenuBuilder.registerType( "ClassicWeaponUnlockButton", ClassicWeaponUnlockButton )
LockTable( _M )
