local f0_local0 = module
local f0_local1, f0_local2 = ...
f0_local0( f0_local1, package.seeall )
function PostLoadFunc( f1_arg0, f1_arg1, f1_arg2 )
	local f1_local0 = 0.75
	f1_arg0:SubscribeToModel( DataSources.inGame.player.currentWeapon.adsPercent:GetModel( f1_arg1 ), function ( f2_arg0 )
		local f2_local0 = DataModel.GetModelValue( f2_arg0 )
		if f1_arg0.isVisible ~= true and f1_local0 <= f2_local0 then
			ACTIONS.AnimateSequence( f1_arg0, "ADSIN" )
			f1_arg0.isVisible = true
		elseif f1_arg0.isVisible ~= false and f2_local0 < f1_local0 then
			ACTIONS.AnimateSequence( f1_arg0, "ADSOUT" )
			f1_arg0.isVisible = false
		end
	end )
end

function Reticle35( menu, controller )
	local self = LUI.UIElement.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 64 * _1080p, 0, 64 * _1080p )
	self.id = "Reticle35"
	self._animationSets = {}
	self._sequences = {}
	local f3_local1 = controller and controller.controllerIndex
	if not f3_local1 and not Engine.InFrontend() then
		f3_local1 = self:getRootController()
	end
	assert( f3_local1 )
	local f3_local2 = self
	local Circle = nil
	
	Circle = LUI.UIImage.new()
	Circle.id = "Circle"
	Circle:SetRGBFromInt( 0, 0 )
	Circle:SetAlpha( 0.6, 0 )
	Circle:setImage( RegisterMaterial( "hud_reticle_full_circle" ), 0 )
	Circle:SetAnchorsAndPosition( 0, 0, 0, 0, _1080p * 13.5, _1080p * -13.5, _1080p * 13.5, _1080p * -13.5 )
	self:addElement( Circle )
	self.Circle = Circle
	
	local ImageS = nil
	
	ImageS = LUI.UIImage.new()
	ImageS.id = "ImageS"
	ImageS:SetAlpha( 0.4, 0 )
	ImageS:setImage( RegisterMaterial( "hud_reticle_35" ), 0 )
	ImageS:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -23, _1080p * 23, _1080p * -23, _1080p * 23 )
	self:addElement( ImageS )
	self.ImageS = ImageS
	
	local Image = nil
	
	Image = LUI.UIImage.new()
	Image.id = "Image"
	Image:SetAlpha( 0.6, 0 )
	Image:setImage( RegisterMaterial( "hud_reticle_35" ), 0 )
	Image:SetBlendMode( BLEND_MODE.addWithAlpha )
	Image:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -23, _1080p * 23, _1080p * -23, _1080p * 23 )
	self:addElement( Image )
	self.Image = Image
	
	local NotchBR = nil
	
	NotchBR = LUI.UIImage.new()
	NotchBR.id = "NotchBR"
	NotchBR:SetRGBFromInt( 0, 0 )
	NotchBR:SetAlpha( 0.6, 0 )
	NotchBR:SetZRotation( 90, 0 )
	NotchBR:setImage( RegisterMaterial( "hud_reticle_notch" ), 0 )
	NotchBR:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -3, _1080p * 3, _1080p * -24, _1080p * -17 )
	self:addElement( NotchBR )
	self.NotchBR = NotchBR
	
	local NotchTL = nil
	
	NotchTL = LUI.UIImage.new()
	NotchTL.id = "NotchTL"
	NotchTL:SetRGBFromInt( 0, 0 )
	NotchTL:SetAlpha( 0.6, 0 )
	NotchTL:SetZRotation( 90, 0 )
	NotchTL:setImage( RegisterMaterial( "hud_reticle_notch" ), 0 )
	NotchTL:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -3, _1080p * 3, _1080p * 17, _1080p * 24 )
	self:addElement( NotchTL )
	self.NotchTL = NotchTL
	
	self._animationSets.DefaultAnimationSet = function ()
		self._sequences.DefaultSequence = function ()
			
		end
		
		ImageS:RegisterAnimationSequence( "ADSIN", {
			{
				function ()
					return self.ImageS:SetAlpha( 0.8, 60 )
				end
			}
		} )
		Image:RegisterAnimationSequence( "ADSIN", {
			{
				function ()
					return self.Image:SetAlpha( 0.8, 60 )
				end
			}
		} )
		self._sequences.ADSIN = function ()
			ImageS:AnimateSequence( "ADSIN" )
			Image:AnimateSequence( "ADSIN" )
		end
		
		ImageS:RegisterAnimationSequence( "ADSOUT", {
			{
				function ()
					return self.ImageS:SetAlpha( 0.2, 60 )
				end
			}
		} )
		Image:RegisterAnimationSequence( "ADSOUT", {
			{
				function ()
					return self.Image:SetAlpha( 0.1, 0 )
				end,
				function ()
					return self.Image:SetAlpha( 0.4, 60 )
				end
			}
		} )
		self._sequences.ADSOUT = function ()
			ImageS:AnimateSequence( "ADSOUT" )
			Image:AnimateSequence( "ADSOUT" )
		end
		
		NotchBR:RegisterAnimationSequence( "OnFire", {
			{
				function ()
					return self.NotchBR:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -3, _1080p * 3, _1080p * -34, _1080p * -27, 100 )
				end,
				function ()
					return self.NotchBR:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -3, _1080p * 3, _1080p * -24, _1080p * -17, 200, LUI.EASING.inCubic )
				end
			}
		} )
		NotchTL:RegisterAnimationSequence( "OnFire", {
			{
				function ()
					return self.NotchTL:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -3, _1080p * 3, _1080p * 27, _1080p * 34, 100 )
				end,
				function ()
					return self.NotchTL:SetAnchorsAndPosition( 0.5, 0.5, 0.5, 0.5, _1080p * -3, _1080p * 3, _1080p * 17, _1080p * 24, 200, LUI.EASING.inCubic )
				end
			}
		} )
		self._sequences.OnFire = function ()
			NotchBR:AnimateSequence( "OnFire" )
			NotchTL:AnimateSequence( "OnFire" )
		end
		
	end
	
	self._animationSets.DefaultAnimationSet()
	self:SubscribeToModel( DataSources.inGame.player.currentWeapon.shotCounter:GetModel( f3_local1 ), function ()
		ACTIONS.AnimateSequence( self, "OnFire" )
	end )
	PostLoadFunc( self, f3_local1, controller )
	return self
end

MenuBuilder.registerType( "Reticle35", Reticle35 )
LockTable( _M )
