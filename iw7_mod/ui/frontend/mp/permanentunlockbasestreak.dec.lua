...
function PostLoadFunc( f1_arg0, f1_arg1, f1_arg2 )
	local f1_local0 = function ()
		...
		if f2_local0 ~= nil then
			if (tonumber( Cac.GetStreakUnlockRank( f2_local0 ) ) == 0) or Prestige.IsItemUnlocked( f1_arg1, f2_local0, "killstreak" ) then
	...
	if Engine.IsPC() then
		f1_arg0:addEventHandler( "button_action", function ( f4_arg0, f4_arg1 )
			f1_arg0:dispatchEventToCurrentMenu( {
				name = "streak_select_alternate"
			} )
		end )
	end
end

function PermanentUnlockBaseStreak( menu, controller )
	local self = LUI.UIButton.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 150 * _1080p, 0, 150 * _1080p )
	self.id = "PermanentUnlockBaseStreak"
	...
	Base:SubscribeToModelThroughElement( self, "streak.image", function ()
		...
		f6_local0 = f6_local0.streak.image:GetValue( f5_local1 )
	...
	Base:SubscribeToModelThroughElement( self, "streak.name", function ()
		...
		f7_local0 = f7_local0.streak.name:GetValue( f5_local1 )
...
MenuBuilder.registerType( "PermanentUnlockBaseStreak", PermanentUnlockBaseStreak )
LockTable( _M )
