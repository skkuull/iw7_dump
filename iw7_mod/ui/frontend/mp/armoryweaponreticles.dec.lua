...
f0_local0 = "frontEnd.MP.armory.reticles"
function PopFunc()
	WipeGlobalModelsAtPath( f0_local0 )
end

function GetReticleDecorator( f2_arg0, f2_arg1 )
	return function ( f3_arg0, f3_arg1, f3_arg2 )
		assert( f3_arg1.ref )
		assert( f3_arg1.name )
		assert( f3_arg1.image )
		local f3_local0 = ARMORY.lootReticle[tostring( f2_arg1 )][f3_arg1.ref:GetValue( f3_arg2 )]
		assert( f3_local0 )
...
f0_local1 = function ( f5_arg0 )
	return function ( f6_arg0, f6_arg1, f6_arg2 )
		return {
			name = f6_arg1.name,
			reticles = f6_arg1.items:Decorate( GetReticleDecorator( f5_arg0, f6_arg0 + 1 ) ),
...
function PostLoadFunc( f8_arg0, f8_arg1, f8_arg2 )
	...
	local f8_local0 = {
		categoryRef = ARMORY.categoryRefs.RETICLE,
		lootItem = ARMORY.lootReticle,
		itemRowList = ARMORY.reticleRowList,
		itemTable = CSV.reticleTable,
		lootTable = CSV.cosmeticLootMaster
	}
	f8_arg0.MenuTitle.MenuTitle:setText( Engine.Localize( "MENU_RETICLE_CAPS" ), 0 )
...
function ArmoryWeaponReticles( menu, controller )
	local self = LUI.UIElement.new()
	self.id = "ArmoryWeaponReticles"
	...
	QualityItems:SetGridDataSourceThroughElement( Tabs.Tabs, "reticles" )
...
MenuBuilder.registerType( "ArmoryWeaponReticles", ArmoryWeaponReticles )
LUI.FlowManager.RegisterStackPopBehaviour( "ArmoryWeaponReticles", PopFunc )
LockTable( _M )
