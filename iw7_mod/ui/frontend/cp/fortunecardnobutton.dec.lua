...
function fortuneCardNoButton( menu, controller )
	local self = LUI.UIButton.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 173 * _1080p, 0, 239 * _1080p )
	self.id = "fortuneCardNoButton"
	self._animationSets = {}
	self._sequences = {}
	...
	numOwned:SubscribeToModelThroughElement( self, "numOwned", function ()
		...
		f4_local0 = f4_local0.numOwned:GetValue( f1_local1 )
		if f4_local0 ~= nil then
			numOwned:setText( f4_local0, 0 )
		end
	end )
	...
	CardName:SubscribeToModelThroughElement( self, "name", function ()
		...
		if f5_local0 ~= nil then
			CardName:setText( ToUpperCase( LocalizeString( f5_local0 ) ), 0 )
		end
	end )
	...
	self:addEventHandler( "button_over", function ( f26_arg0, f26_arg1 )
		local f26_local0 = f26_arg1.controller or f1_local1
		ACTIONS.AnimateSequence( self, "ButtonOver" )
	end )
	self:addEventHandler( "button_up", function ( f27_arg0, f27_arg1 )
		local f27_local0 = f27_arg1.controller or f1_local1
	...
	self:SubscribeToModelThroughElement( self, "isUnlocked", function ()
		...
		if f28_local0.isUnlocked:GetValue( f1_local1 ) ~= nil then
			f28_local0 = self:GetDataSource()
			if f28_local0.isUnlocked:GetValue( f1_local1 ) == true then
		...
		if f28_local0.isUnlocked:GetValue( f1_local1 ) ~= nil then
			f28_local0 = self:GetDataSource()
			if f28_local0.isUnlocked:GetValue( f1_local1 ) == false then
	...
	self:SubscribeToModelThroughElement( self, "backing", function ()
		...
		if f29_local0.backing:GetValue( f1_local1 ) ~= nil then
			f29_local0 = self:GetDataSource()
			if f29_local0.backing:GetValue( f1_local1 ) == "zm_wc_fortune_card_base" then
		...
		if f29_local0.backing:GetValue( f1_local1 ) ~= nil then
			f29_local0 = self:GetDataSource()
			if f29_local0.backing:GetValue( f1_local1 ) == "zm_wc_fate_card_base" then
				ACTIONS.AnimateSequence( self, "hideOwned" )
			end
		end
	end )
	self:addEventHandler( "button_action", function ( f30_arg0, f30_arg1 )
		local f30_local0 = f30_arg1.controller or f1_local1
		local f30_local1 = self:GetDataSource()
		if f30_local1.isUnlocked:GetValue( f30_local0 ) ~= nil then
			f30_local1 = self:GetDataSource()
			if f30_local1.isUnlocked:GetValue( f30_local0 ) == true then
				f30_local1 = self:GetDataSource()
				if f30_local1.backing:GetValue( f30_local0 ) ~= nil then
					f30_local1 = self:GetDataSource()
					if f30_local1.backing:GetValue( f30_local0 ) == "zm_wc_fortune_card_base" then
						f30_local1 = self:GetDataSource()
						if f30_local1.numOwned:GetValue( f30_local0 ) ~= nil then
							f30_local1 = self:GetDataSource()
							if f30_local1.numOwned:GetValue( f30_local0 ) > 0 then
								ACTIONS.CardSetPlayerData( self, f30_arg0, f30_local0 )
...
MenuBuilder.registerType( "fortuneCardNoButton", fortuneCardNoButton )
LockTable( _M )
